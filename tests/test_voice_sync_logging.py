...
def _mk_segs(n, slot=1.0, spacing=1.2):
    return [
        {"id": i + 1, "start": i * spacing, "end": i * spacing + slot, "duration": slot}
        for i in range(n)
    ]
...
def test_log_contains_required_fields(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="autodub.timing"):
        apply_soft_timing(segments, str(seg_dir), str(tmp_path / "timed"), Settings())
    voice_sync = [r for r in caplog.records if "[VOICE-SYNC]" in r.getMessage()]
...
def test_fitted_and_overlapped_always_logged(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="autodub.timing"):
        apply_soft_timing(segments, str(seg_dir), str(tmp_path / "timed"), Settings())
    voice_sync = [r.getMessage() for r in caplog.records if "[VOICE-SYNC]" in r.getMessage()]
