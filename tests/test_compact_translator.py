...
def test_is_translation_too_long():
    ...
    assert is_translation_too_long(
        "Hôm nay tôi cảm thấy vô cùng vui vẻ và hạnh phúc",
        slot_duration_s=1.0,
        max_syllables_per_s=4.5,
    )
...
def test_compact_vietnamese_text_offline_heuristic():
    ...
    assert estimate_vietnamese_syllable_count(compacted) <= estimate_vietnamese_syllable_count(
        long_text
    )
