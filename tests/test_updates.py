...
from autodub.updates import check_for_update, is_newer, parse_version
...
def test_check_has_update(monkeypatch):
    ...
    fake_resp = types.SimpleNamespace(
        raise_for_status=lambda: None,
        json=lambda: {"tag_name": "v2.2", "html_url": "https://gh/releases", "body": "Bug fixes"},
    )
    import requests
...
def test_check_invalid_repo(monkeypatch):
    ...
    monkeypatch.setattr(
        requests, "get", lambda *a, **kw: (_ for _ in ()).throw(AssertionError("should not call"))
    )
