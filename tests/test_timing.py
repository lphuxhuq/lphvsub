    ...
    return [
        {
            "id": i + 1,
            "start": float(s),
            **({"end": float(s) + dur, "duration": float(dur)} if dur else {}),
        }
...
def test_case7_silence_gap_utilised():
    ...
    segs = [
        {"id": 1, "start": 8.0, "end": 10.0, "duration": 2.0},
...
def test_drift_capped_and_tempo_bounded():
    ...
    p, r = plan_voice_placements(segs, [3.5] * 5, max_start_drift_s=0.15, max_speed=1.15)
    for seg, placed in zip(segs, p):
    ...
    assert r.segments_overlapped > 0 or any(pl["reason"] == "needs_compaction" for pl in p)
...
def test_apply_soft_timing_mutates_timeline(tmp_path):
    ...
    settings = Settings()
    out_dir, report = apply_soft_timing(segments, str(seg_dir), str(tmp_path / "timed"), settings)
    ...
    assert segs_overlap[1]["start"] <= 3.0 + settings_allow_overlap.timing_max_start_drift_s + 1e-6
