...
def test_not_installed_when_custom_voices_missing(settings, tmp_path, monkeypatch):
    ...
    (voices_dir / voice_downloader.MANIFEST_NAME).write_text("{}", encoding="utf-8")
    assert voice_downloader.voices_installed(settings) is False
...
def test_installed_when_manifest_and_enough_presets(settings, tmp_path, monkeypatch):
    monkeypatch.setattr(voice_downloader, "app_root", lambda: str(tmp_path))
    voices_dir = tmp_path / voice_downloader.VOICES_TARGET_DIR
    voices_dir.mkdir(parents=True)
    (voices_dir / voice_downloader.MANIFEST_NAME).write_text("{}", encoding="utf-8")
...
def test_bundled_embeddings_skip_enroll(settings, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        voice_downloader, "download_voices", lambda cb=None: str(tmp_path / "voices.zip")
    )
    monkeypatch.setattr(voice_downloader, "extract_voices", lambda z: str(voices_dir))
...
def test_slim_zip_contract_no_wav_still_counts_as_installed(settings, tmp_path, monkeypatch):
    ...
    (voices_dir / voice_downloader.MANIFEST_NAME).write_text("[]", encoding="utf-8")
    write_custom(
        settings, {f"Giọng {i}": {"gender": "male", "source": "library"} for i in range(60)}
    )
