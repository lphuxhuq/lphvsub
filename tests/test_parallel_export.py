...
class TestPlanChunkBoundaries:
    ...
    def test_min_gap_respected(self):
        ...
        bounds = plan_chunk_boundaries(kfs, 200.0, target_chunks=8, min_chunk_s=12.0)
        for a, b in zip(bounds, bounds[1:]):
            assert b - a >= 12.0
    ...
    def test_keyframes_outside_range_ignored(self):
        kfs = [-5.0, 0.0, 300.0]
        bounds = plan_chunk_boundaries(kfs, 100.0, target_chunks=2, min_chunk_s=10.0)
...
class TestWriteConcatList:
    def test_writes_file_uris(self, tmp_path):
        ...
        assert (
            str(c1.resolve()) in content.replace("\\", "/")
            or os.path.abspath(str(c1)).replace("\\", "/") in content
        )
...
class TestParallelChunkedExport:
    ...
    def test_success_flow_produces_output(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("autodub.media.parallel_export.probe_fps", lambda p: 30.0)
    ...
    def test_tmp_dir_cleaned(self, tmp_path, monkeypatch):
        ...
        assert not [
            d for d in leftovers_after - leftovers_before if d.startswith("autodub_pexport_")
        ]
...
class TestProbeKeyframes:
    def test_raises_when_no_keyframe(self, monkeypatch):
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: mock.MagicMock(returncode=0, stdout="0.0,__\n0.5,___\n"),
        )
        with pytest.raises(ParallelExportError):
    ...
    def test_raises_on_ffprobe_failure(self, monkeypatch):
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: mock.MagicMock(returncode=1, stdout="", stderr="bad file"),
        )
        with pytest.raises(ParallelExportError):
            probe_keyframes("fake.mp4")
    ...
    def test_parses_k_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: mock.MagicMock(returncode=0, stdout=stdout, stderr=""),
        )
    ...
    def test_parses_r_frame_rate(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda *a, **k: mock.MagicMock(
                returncode=0, stdout='{"streams": [{"r_frame_rate": "30000/1001"}]}', stderr=""
            ),
        )
