...
class Stepper(QWidget):
    ...
    def set_live_mode(self, live: bool) -> None:
        ...
        self.setCursor(Qt.CursorShape.ArrowCursor if live else Qt.CursorShape.PointingHandCursor)
        self.update()
    ...
    def _paint_connectors(self, painter: QPainter, centers: list[float], cy: float) -> None:
        ...
        for i in range(len(centers) - 1):
            ...
            painter.drawLine(
                int(centers[i] + _CIRCLE / 2 + 4),
                int(cy),
                int(centers[i + 1] - _CIRCLE / 2 - 4),
                int(cy),
            )

    def _paint_circle(self, painter: QPainter, index: int, cx: float, cy: float) -> None:
        ...
        fill = {DONE: _DONE_BG, CURRENT: tokens.PRIMARY, UPCOMING: _UPCOMING_BG}[state]
        ...
        if state == DONE:
            ...
            painter.setPen(QColor(tokens.TEXT_ON_ACCENT if state == CURRENT else _UPCOMING_TEXT))
            painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, str(index + 1))

    def _paint_label(
        self, painter: QPainter, index: int, cx: float, cy: float, slot: float
    ) -> None:
        ...
        font.setWeight(QFont.Weight.DemiBold if state == CURRENT else QFont.Weight.Normal)
        painter.setFont(font)
        painter.setPen(QColor(color))
        rect = QRectF(
            cx - slot / 2 + 2,
            cy + _CIRCLE / 2 + _LABEL_GAP,
            slot - 4,
            _HEIGHT - cy - _CIRCLE / 2 - _LABEL_GAP,
        )
