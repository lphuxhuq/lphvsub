...
class _StateBase(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._root.setContentsMargins(tokens.SP_6, tokens.SP_6, tokens.SP_6, tokens.SP_6)
        self._root.setSpacing(tokens.SP_3)
        self._root.addStretch()

    def _add_title(
        self, text: str, color: str = tokens.TEXT_SECONDARY, size: int = tokens.FS_CARD_TITLE
    ) -> QLabel:
        lbl = QLabel(text)
        lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
...
class LoadingState(_StateBase):
    ...
    def __init__(self, text: str = "Đang tải dữ liệu", parent: QWidget | None = None):
        super().__init__(parent)
        self._base = text
        self._label = self._add_title(text, color=tokens.TEXT_SECONDARY, size=tokens.FS_BODY)
        self._close()
        self._n = 0
...
class ErrorState(_StateBase):
    ...
    def __init__(
        self,
        title: str = "Không mở được dữ liệu",
        description: str = "",
        parent: QWidget | None = None,
        *,
        retry_label: str = "Thử lại",
        action_label: str = "",
    ):
        super().__init__(parent)
