...
def styled_label(
    text: str,
    *,
    size: int = tokens.FS_BODY,
    color: str = tokens.TEXT_PRIMARY,
    weight: int = 400,
    wrap: bool = False,
    parent: QWidget | None = None,
) -> QLabel:
...
def title_label(text: str, parent: QWidget | None = None) -> QLabel:
    ...
    return styled_label(
        text, size=tokens.FS_SECTION, color=tokens.TEXT_PRIMARY, weight=700, parent=parent
    )
...
def caption_label(text: str, parent: QWidget | None = None) -> QLabel:
    ...
    return styled_label(
        text, size=tokens.FS_META, color=tokens.TEXT_MUTED, weight=400, parent=parent
    )
