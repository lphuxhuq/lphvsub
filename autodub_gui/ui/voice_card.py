...
class VoiceCard(QFrame):
    ...
    def _build(self) -> None:
        ...
        root.setContentsMargins(tokens.SP_4, tokens.SP_4, tokens.SP_4, tokens.SP_3)
        ...
        sub_parts = [
            country_label.get(self.voice.country, ""),
        ...
        if gender_text:
            badge = StatusBadge(
                gender_text, "accent" if self.voice.gender == "female" else "processing"
            )
        ...
        self.btn_preview.clicked.connect(lambda: self.preview_requested.emit(self.voice.name))
