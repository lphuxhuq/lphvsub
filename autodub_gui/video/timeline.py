...
class TimelineCanvas(QWidget):
    ...
    def set_peaks(self, values: list[float]) -> None:
        ...
        self._loading = False
        self.setToolTip("" if self._peaks else "Không đọc được dạng sóng của tệp âm thanh này")
        self.update()
    ...
    def _band_kinds(self) -> list[str]:
        ...
        kinds = [
            k
            for k, *_ in _TRACK_META
            if self._track_avail.get(k) and self._track_visible.get(k, True)
        ]
    ...
    def set_segments(self, segments: list[dict]) -> None:
        self._segments = sorted(segments, key=lambda s: float(s.get("start", 0.0)))
        self.update()
    ...
    def set_scissors(self, on: bool) -> None:
        ...
        self._scissors = on
        self.setCursor(Qt.CursorShape.SplitHCursor if on else Qt.CursorShape.ArrowCursor)
    ...
    def _to_time(self, x: float) -> float:
        span = self._visible_span()
        return self._offset + max(0.0, x - LABEL_W) / self._content_w() * span
    ...
    def _paint_placeholder(self, painter: QPainter) -> None:
        ...
        painter.drawText(self.rect(), Qt.AlignmentFlag.AlignCenter, "Chưa có dự án nào được mở")
    ...
    def _paint_ruler(self, painter: QPainter) -> None:
        ...
        if self._selection:
            ...
            sel_rect = QRectF(max(xs, LABEL_W), 0, max(0.0, xe - max(xs, LABEL_W)), RULER_H)
        ...
        while tick <= end:
            ...
            painter.drawText(QPointF(x + 3, RULER_H - 8), format_duration(tick))
            tick += step
    ...
    def _paint_thumbnails(self, painter: QPainter) -> None:
        ...
        painter.fillRect(QRectF(LABEL_W, top, self._content_w(), THUMB_H), QColor(tokens.BG_VIDEO))
    ...
    def _paint_waveform(self, painter: QPainter) -> None:
        ...
        for kind in kinds:
            if kind == "default":
                self._paint_band(
                    painter, top, self._peaks, tokens.WAVEFORM, tokens.BG_PANEL, show_hint=True
                )
            else:
    ...
    def _paint_labels(self, painter: QPainter) -> None:
        ...
        painter.fillRect(QRectF(0, 0, LABEL_W, height), QColor(tokens.TRACK_LABEL_BG))
        ...
        if thumb_top >= 0:
            ...
            painter.drawText(
                QRectF(4, thumb_top, LABEL_W - 8, THUMB_H), Qt.AlignmentFlag.AlignVCenter, "Video"
            )
        ...
        painter.setPen(QColor(tokens.TEXT_MUTED))
        painter.drawText(
            QRectF(24, top, LABEL_W - 28, TRACK_H), Qt.AlignmentFlag.AlignVCenter, "Câu thoại"
        )
        # Vẽ nút toggle cho các track có sẵn nhưng bị ẩn (ở cuối cột)
        hidden = [
            k
            for k, *_ in _TRACK_META
            if self._track_avail.get(k) and not self._track_visible.get(k, True)
        ]
        if hidden:
            toggle_top = height - len(hidden) * 20 - 4
            for kind in hidden:
                meta = next(m for m in _TRACK_META if m[0] == kind)
                self._paint_toggle_btn(painter, 4, toggle_top, kind, meta[1], False)
                toggle_top += 20

    def _paint_track_label(self, painter: QPainter, top: float, kind: str) -> None:
        ...
        painter.drawText(QRectF(24, top, LABEL_W - 48, BAND_H), Qt.AlignmentFlag.AlignVCenter, text)
        # Nút toggle (mắt) nếu không phải default
        if kind != "default":
            visible = self._track_visible.get(kind, True)
            self._paint_toggle_btn(painter, LABEL_W - 20, top + BAND_H / 2 - 8, kind, text, visible)

    def _paint_toggle_btn(
        self, painter: QPainter, x: float, y: float, kind: str, tooltip: str, visible: bool
    ) -> None:
        """Vẽ nút toggle hiển thị track (icon mắt 16×16)."""
        from autodub_gui import icons

        icon = icons.eye(tokens.TEXT_MUTED) if visible else icons.eye_off(tokens.TEXT_DISABLED)
    ...
    def _paint_segments(self, painter: QPainter) -> None:
        ...
        painter.fillRect(QRectF(0, top, self.width(), TRACK_H), QColor(tokens.BG_MAIN))
        visible_end = self._offset + self._visible_span()
        font = QFont(self.font())
        font.setPixelSize(tokens.FS_BADGE)
        painter.setFont(font)
    ...
    def _paint_block(
        self, painter: QPainter, segment: dict, start: float, end: float, top: float
    ) -> None:
        ...
        painter.setPen(
            QPen(
                QColor(tokens.BORDER_ACTIVE if selected else tokens.SUB_BLOCK_BORDER),
        ...
        if rect.width() > 30:
            ...
            text = painter.fontMetrics().elidedText(
                str(segment.get("text_vi", "")), Qt.TextElideMode.ElideRight, int(rect.width()) - 8
            )
    ...
    def _paint_playhead(self, painter: QPainter) -> None:
        ...
        half = _HANDLE_W / 2
        painter.drawPolygon(
            QPolygonF([QPointF(x - half, 0), QPointF(x + half, 0), QPointF(x, _HANDLE_W)])
        )
    ...
    def _segment_at(self, x: float, y: float) -> dict | None:
        ...
        seconds = self._to_time(x)
        for segment in self._segments:
            if float(segment.get("start", 0.0)) <= seconds <= float(segment.get("end", 0.0)):
                return segment
        return None
    ...
    def _limit_to_neighbours(self, seg_id: int, start: float, end: float) -> tuple[float, float]:
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        for segment in self._segments:
            if segment.get("id") == drag["id"]:
                ...
                if abs(new_start - drag["start"]) > 1e-6 or abs(new_end - drag["end"]) > 1e-6:
    ...
    def wheelEvent(self, event) -> None:
        ...
        if event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            ...
            self.set_zoom(self._zoom * (_ZOOM_STEP if delta > 0 else 1 / _ZOOM_STEP), anchor)
            return
...
class Timeline(QWidget):
    ...
    def _update_height(self) -> None:
        ...
        self.setFixedHeight(RULER_H + self.canvas.wave_height() + TRACK_H + TOOLBAR_H + 12)
    ...
    def _on_selection_changed(self, start: float, end: float) -> None:
        ...
        self.selection_label.setText(f"{format_duration(start)} – {format_duration(end)}")
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        self.btn_clear_selection = IconButton(
            icons.error(tokens.TEXT_MUTED), "Bỏ vùng chọn", size=26
        )
        ...
        self.selection_label.setStyleSheet(
            f"color: {tokens.PRIMARY}; font-size: {tokens.FS_BADGE}px; background: transparent;"
        )
        self.selection_label.hide()
        ...
        for icon, tip, handler in (
            (icons.zoom_out(tokens.TEXT_SECONDARY), "Thu nhỏ", self._zoom_out),
            (icons.zoom_in(tokens.TEXT_SECONDARY), "Phóng to", self._zoom_in),
        ):
