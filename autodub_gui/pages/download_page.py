...
class DownloadPage(BasePage):
    ...
    def _build_input_card(self) -> QWidget:
        ...
        self.output = FilePicker(
            "Thư mục lưu", "downloads", "Nơi lưu các video tải về", directory=True
        )
    ...
    def _confirm_disclaimer(self) -> bool:
        ...
        accepted, dont_ask = ConfirmDialog.ask(
            ...
            kind="info",
            confirm_label="Tôi hiểu, tiếp tục tải",
            cancel_label="Thôi",
            checkbox_label="Không hỏi lại trên máy này",
            checkbox_checked=True,
        )
        if accepted and dont_ask:
            try:
                with open(_disclaimer_marker_path(), "w", encoding="utf-8") as f:
                    f.write("accepted\n")
            except OSError:
    ...
    def _start(self) -> None:
        ...
        if not urls:
            TOASTS.warn(
                "Chưa có liên kết nào hợp lệ. Mỗi dòng cần một liên "
        ...
        worker = DownloadWorker(
            urls, self.output.text() or "downloads", self.cookies.current_key(), None, self
        )
    ...
    def _on_item_status(self, _index: int, _total: int, url: str, status: str, detail: str) -> None:
    ...
    def _on_finished(self, success: int, failed: int) -> None:
        ...
        self.summary.setText(
            f"Đã tải xong {success} video" + (f", {failed} liên kết lỗi" if failed else "")
        )
        TOASTS.success(
            f"Tải xong {success} video.", action_label="Mở thư mục", on_action=self._open_output
        )
    ...
    def _on_failed(self, message: str) -> None:
        ...
        ConfirmDialog.show_error(
            ...
            "khác mà bạn có quyền ghi, rồi thử lại.",
            detail=message,
        )
    ...
    def _actions(self, url: str, status: str, detail: str) -> list[QWidget]:
        ...
        if status == "success" and detail:
            first = IconButton(icons.play(tokens.SUCCESS), "Mở video", size=_ACTION_ICON)
            first.clicked.connect(lambda _c=False, p=detail: self._open(p))
            second = IconButton(
                icons.folder(tokens.TEXT_SECONDARY), "Mở thư mục chứa video", size=_ACTION_ICON
            )
            second.clicked.connect(lambda _c=False, p=detail: reveal_file(p))
            return [first, second]
        first = IconButton(
            icons.external(tokens.TEXT_SECONDARY),
