...
def _star(on: bool, button: IconButton | None = None) -> IconButton:
    ...
    """
    if button is None:
        button = IconButton(icons.star(tokens.TEXT_MUTED), "", size=_ROW_BTN, checkable=True)
    button.setChecked(on)
    button.setIcon(icons.star(tokens.WARNING if on else tokens.TEXT_MUTED))
    button.setToolTip("Bỏ khỏi danh sách yêu thích" if on else "Đánh dấu giọng yêu thích")
    return button
...
def _labels() -> tuple[dict, dict, dict]:
    ...
    return ({k: v for v, k in GENDERS}, {k: v for v, k in COUNTRIES}, {k: v for v, k in STYLES})
...
class _Waveform(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        for i in range(_WAVE_BARS):
            ...
            painter.setPen(
                QPen(QColor(color), pen_w, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
            )
            x = step * (i + 0.5)
...
class _VoiceRow(QFrame):
    ...
    def _build(self, favorite: bool) -> None:
        ...
        self.check.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents)
        ...
        name_box.addWidget(_text(name, tokens.TEXT_PRIMARY, tokens.FS_BODY, 600))
        if gender:
            name_box.addWidget(
                _pill(gender, tokens.TEXT_SECONDARY, tokens.BORDER_SUBTLE, tokens.CHIP_BG),
                0,
        ...
        info = _SEP.join(p for p in (countries.get(self.voice.country, ""), gender) if p)
        row.addWidget(_text(info, tokens.TEXT_SECONDARY, tokens.FS_LABEL), _COLS[1])
        ...
        actions_w.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, False)
        ...
        self.btn_play = IconButton(
            icons.play(tokens.SUCCESS), f"Nghe thử giọng «{name}»", size=_ROW_BTN
        )
        self.btn_play.clicked.connect(lambda: self.preview_requested.emit(name))
    ...
    def _apply_style(self) -> None:
        background = tokens.VOICE_SELECTED_BG if self._selected else tokens.BG_PANEL
        border = tokens.BORDER_ACTIVE if self._selected else tokens.BORDER_SUBTLE
        hover = (
            ""
            if self._selected
            else (
...
class VoiceLibraryTab(QWidget):
    ...
    def __init__(self, settings_provider, parent: QWidget | None = None):
        ...
        self._preview.finished.connect(lambda _ok: self._set_preview_enabled(True))
        self._voices: list = []
        self._filtered: list = []
    ...
    def _build_header(self) -> QWidget:
        ...
        for title, stretch in zip(_HEADERS, _COLS):
            label = _text(title.upper(), tokens.SECTION_LABEL, tokens.FS_BADGE, 700)
    ...
    def _build_rail(self) -> QWidget:
        ...
        col.addWidget(_text("Giọng đọc đã chọn", tokens.TEXT_PRIMARY, tokens.FS_SECTION, 600))
        ...
        self._rail_name = _text("", tokens.TEXT_PRIMARY, tokens.FS_SECTION, 700)
        ...
        col.addWidget(_text("Phong cách", tokens.TEXT_SECONDARY, tokens.FS_LABEL))
        ...
        self._use.setToolTip("Ghi nhớ giọng này cho dự án mới (bấm Lưu ở cuối trang).")
        self._use.clicked.connect(self._on_use)
        col.addWidget(self._use)

        col.addWidget(_text("Giọng đọc gần đây", tokens.TEXT_SECONDARY, tokens.FS_LABEL))
    ...
    def _build_preview_box(self) -> QWidget:
        ...
        inner.setContentsMargins(tokens.SP_3, tokens.SP_3, tokens.SP_3, tokens.SP_3)
        inner.setSpacing(tokens.SP_2)
        inner.addWidget(_text("Nghe thử giọng", tokens.TEXT_SECONDARY, tokens.FS_LABEL))
        wave_row = QHBoxLayout()
        wave_row.setSpacing(tokens.SP_3)
        self._rail_play = IconButton(
            icons.play(tokens.SUCCESS), "Nghe thử giọng đang chọn", size=_RAIL_BTN
        )
    ...
    def _apply_filters(self) -> None:
        ...
        if self._tab == _TAB_RECENT:
            ...
            matched = sorted((v for v in matched if v.name in rank), key=lambda v: rank[v.name])
        elif self._sort.currentData() == "custom":
            matched.sort(key=lambda v: (not v.custom, v.name.lower()))
        else:
            matched.sort(
                key=lambda v: v.name.lower(), reverse=self._sort.currentData() == "name_desc"
            )
    ...
    def _update_tabs(self) -> None:
        ...
        if self._src_tabs.isVisible():
            capcut = sum(1 for v in self._voices if source_group(v) == "capcut")
            src_counts = (len(self._voices) - capcut, capcut)
            for button, label, count in zip(self._src_tab_buttons, _SRC_LABELS, src_counts):
                button.setText(f"{label} ({count})")
        self._online_hint.setVisible(self._src_tabs.isVisible() and self._src_tab == _SRC_CAPCUT)
    ...
    def _rebuild_rows(self) -> None:
        ...
        self._count.setText(f"{total} giọng" if shown == total else f"{shown} trên {total} giọng")
    ...
    def _push_recent(self, name: str) -> None:
        self._recent = [name, *(n for n in self._recent if n != name)][:_RECENT_MAX]
    ...
    def _update_rail(self) -> None:
        ...
        if not self._recent:
            self._recent_row.addWidget(_muted("Giọng bạn chọn sẽ được ghi nhớ ở đây."))
            return
    ...
    def _tag_text(voice) -> str:
        ...
        return _SEP.join(
            p
            for p in (
                countries.get(voice.country, ""),
                genders.get(voice.gender, ""),
                styles.get(voice.style, ""),
                "CapCut" if voice.is_capcut else "",
            )
            if p
        )
