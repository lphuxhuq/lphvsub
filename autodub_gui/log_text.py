    ...
    def narrate(self, event) -> tuple[str, int, bool] | None:
        ...
        if status == "progress":
            ...
            if not template or not total:
                return None
            text = template.format(current=getattr(event, "current", 0), total=total)
...
_RAW_NOTICES: list[tuple[str, _Template, int]] = [
    # -- Lấy video ---------------------------------------------------------
    (r"Đang tải video về máy", "Đang tải video từ đường dẫn bạn nhập", logging.INFO),
    (r"Tải trước video kế tiếp", "Đang tải trước video kế tiếp trong hàng chờ", logging.INFO),
    (
        r"Tải trước quá lâu",
        "Tải trước lâu hơn bình thường — video sẽ được tải lại",
        logging.WARNING,
    ),
    (
    ...
    (r"^Segment (\d+): \[([\d.]+)s-([\d.]+)s\] (.+)$", _asr_segment_line, logging.INFO),
    (
        r"Dùng lại lời thoại đã nghe từ lần chạy trước \((\d+) câu",
        "Dùng lại {0} câu thoại đã nghe ở lần chạy trước",
        logging.INFO,
    ),
    (r"Nghe xong: video có (\d+) câu", "Đã nghe xong: video có {0} câu thoại", SUCCESS),
    (
        r"Transcript cũ hỏng",
        "Bản ghi lời thoại cũ không dùng được — nghe lại từ đầu",
        logging.WARNING,
    ),
    ...
    # -- Dịch ---------------------------------------------------------------
    (r"Dùng lại bản dịch đã có", "Dùng lại bản dịch của lần chạy trước", logging.INFO),
    (
        r"Dùng lại phân tích ngữ cảnh video",
        "Dùng lại phần tìm hiểu nội dung video của lần chạy trước",
        logging.INFO,
    ),
    (r"Đang dịch (\d+) câu sang tiếng Việt", "Đang dịch {0} câu sang tiếng Việt", logging.INFO),
    ...
    (r"Máy chủ đang bận", "Máy chủ đang quá tải, chờ một chút rồi thử lại", logging.WARNING),
    (r"Lượt dịch này tốn ([\d.,]+) Vox", "Lượt dịch này tốn {0} Vox", logging.INFO),
    (
        r"câu dịch còn lẫn tiếng Trung — dịch lại",
        "Đang dịch lại vài câu còn lẫn tiếng Trung",
        logging.INFO,
    ),
    (
    ...
    (
        ...
        "Dịch tự động gặp lỗi — chuyển sang chờ bạn dịch tay",
        logging.WARNING,
    ),
    (r"Rà soát bản dịch lỗi|Soát lại câu .* lỗi", None, logging.INFO),
    (r"Soát lại bản dịch: mọi câu đều đạt", "Đã soát lại bản dịch: mọi câu đều đạt", SUCCESS),
    (r"Soát lại bản dịch: (\d+) câu cần sửa", "Đang soát lại {0} câu dịch chưa ổn", logging.INFO),
    (r"Soát lại bản dịch: đã sửa xong (\d+)/(\d+)", "Đã soát và sửa {0}/{1} câu dịch", SUCCESS),
    ...
    (
        ...
        "Đang tạo giọng đọc cho {0} câu — đây là bước lâu nhất",
        logging.INFO,
    ),
    (
    ...
    (
        ...
        "Giọng «{0}», {1}/{2} câu cần đọc, {3} luồng cùng lúc",
        logging.INFO,
    ),
    (
        r"Chỉnh tốc độ giọng đọc ([\d.]+)x",
        "Đang chỉnh tốc độ giọng đọc thành {0}x cho cả video",
        logging.INFO,
    ),
    (
    ...
    (r"Đã lưu (\d+) câu vào", "Đã lưu {0} câu vừa sửa", SUCCESS),
    (r"Câu (\d+): giọng riêng", "Đã đổi giọng riêng cho câu {0}", logging.INFO),
    # -- Âm thanh và hình --------------------------------------------------
    (r"Đang cân chỉnh âm lượng", "Đang cân âm lượng các câu cho đều nhau", logging.INFO),
    (r"Sắp xếp thời gian các câu", "Đang sắp lại thời gian các câu cho khớp hình", logging.INFO),
    (r"Đang ghép giọng đọc với nhạc nền", "Đang ghép giọng đọc với nhạc nền", logging.INFO),
    (
    ...
    (
        ...
        "Máy này xuất video bằng card đồ họa {0} — nhanh hơn nhiều",
        logging.INFO,
    ),
    (r"Đang xuất video hoàn chỉnh", "Đang xuất video hoàn chỉnh — bước này chạy lâu", logging.INFO),
    (
        r"Làm chậm video .* thất bại|Không dùng được bản video đã làm chậm",
        "Không làm chậm được video — giữ nguyên tốc độ gốc",
        logging.WARNING,
    ),
    (r"Đã xuất xong video", "Đã xuất xong video", SUCCESS),
    (r"Đã ghi lại phụ đề vào video", "Đã ghi xong phụ đề vào video", SUCCESS),
    (r"Đã dựng đoạn xem thử câu (\d+)", "Đã dựng xong đoạn xem thử câu {0}", SUCCESS),
    # -- Phụ đề -------------------------------------------------------------
    (r"Đang canh phụ đề", "Đang canh phụ đề chạy khớp giọng đọc", logging.INFO),
    (r"Canh phụ đề xong: (\d+)/(\d+)", "Đã canh phụ đề: {0}/{1} câu khớp chính xác", SUCCESS),
    (
    ...
    (
        ...
        "Đang viết tiêu đề, mô tả và hashtag để đăng bài",
        logging.INFO,
    ),
    (r"Đã viết xong phần đăng bài", "Đã viết xong tiêu đề, mô tả và hashtag", SUCCESS),
    (
        r"Bỏ qua tạo tiêu đề và mô tả|Bỏ qua nội dung đăng bài",
        "Không viết tiêu đề và mô tả (theo lựa chọn của bạn)",
        logging.INFO,
    ),
    (r"Đang vẽ ảnh bìa", "Đang vẽ ảnh bìa", logging.INFO),
    (r"Đã tạo ảnh bìa (\S+)", "Đã vẽ xong ảnh bìa {0}", SUCCESS),
    (r"Bỏ qua vẽ ảnh bìa", "Không vẽ ảnh bìa (chưa bật trong Cài đặt)", logging.INFO),
    (
        r"Tạo ảnh bìa .* lỗi|Ảnh bìa .* không trả về ảnh",
        "Không vẽ được ảnh bìa (không ảnh hưởng video)",
        logging.WARNING,
    ),
    (
    ...
    (
        ...
        "Kiểm tra chất lượng: mọi câu đều khớp đẹp",
        SUCCESS,
    ),
    (
        r"Hoàn tất! Lồng tiếng xong (\d+) câu trong (.+?) —",
        "Hoàn tất: đã lồng tiếng {0} câu trong {1}",
        SUCCESS,
    ),
    (
        r"Đã tự dọn tệp trung gian, giải phóng ([\d.]+) MB",
        "Đã dọn tệp tạm, giải phóng {0} MB",
        logging.INFO,
    ),
]
...
def error_line(message: str) -> tuple[str, int]:
    ...
    return (
        "Dừng lại vì một lỗi ngoài dự tính — xem chi tiết ở hộp thoại "
