...
from autodub.media.download.contract import BandwidthMode, DownloadRequest, DownloadResult
...
from autodub.media.download.retry import SmartRetryPolicy
...
def generate_synthetic_media(output_path: Path, duration_s: int = 2) -> Path:
    cmd = [
        "ffmpeg", "-y",
        "-f", "lavfi", "-i", f"testsrc=duration={duration_s}:size=640x360:rate=30",
...
def run_benchmarks():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"[BENCH] MediaValidator validation time: {t_val*1000:.2f} ms (Valid: {val_res.valid})")
        ...
        print(f"[BENCH] DownloadCache validated lookup time: {t_cache:.3f} ms (Hit: {hit is not None})")
        ...
        results["resume_time_reduction_pct"] = ((old_time_sim - new_time_sim) / old_time_sim) * 100.0
        print(f"[BENCH] Resume efficiency: Cold restart = {old_time_sim:.2f}s vs Smart Resume = {new_time_sim:.2f}s ({results['resume_time_reduction_pct']:.1f}% time saved)")
        ...
        acc = AdaptiveConcurrencyController(bandwidth_mode=BandwidthMode.AUTO, initial_concurrency=4, cooldown_seconds=0.0)
        # Simulate scaling
