...
"""
    ...
    print(
        f"  -> Hoàn thành trong: {dur_baseline:.2f}s | Tốc độ: {fps_baseline:.1f} FPS ({fps_baseline / 30:.2f}x realtime)"
    )
    ...
    print(
        f"  -> Hoàn thành trong: {dur_opt:.2f}s | Tốc độ: {fps_opt:.1f} FPS ({fps_opt / 30:.2f}x realtime)"
    )
