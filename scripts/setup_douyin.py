...
TAG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "python_tag.txt")
...
def step_install_libs() -> None:
    probe = subprocess.run(
        [sys.executable, "-c", f"import sys; sys.path.insert(0, {LIBS_DIR!r}); import playwright"],
        capture_output=True,
    )
