...
def load_custom_voices(tts, path: str) -> int:
    ...
    try:
        ...
        print(f"[vieneu-worker] bỏ qua custom voices hỏng ({e})", file=sys.stderr, flush=True)
        return 0
...
def _kaldi_fbank_numpy(wav, sr: int, n_mels: int = 80):
    ...
    banks = np.maximum(0.0, np.minimum((m - left) / delta, (left + 2 * delta - m) / delta))
...
def _speaker_embed(engine, wav, sr: int):
    ...
    if sr != 16000:
        ...
        wav = soxr.resample(np.asarray(wav, dtype=np.float32).reshape(-1), sr, 16000).astype(
            np.float32
        )
        sr = 16000
    ...
    out = sess.run([sess.get_outputs()[0].name], {sess.get_inputs()[0].name: feat})[0]  # (1, 192)
    return out[0].astype(np.float32)


def _encode_one(tts, wav_path: str, style: str, no_denoise: bool, meta: dict) -> dict:
    ...
    if not no_denoise and engine.denoiser is not None:
        ...
        sr = 44100
    speaker_emb = _speaker_embed(engine, wav, sr) if engine.use_speaker_embedding else None
    ...
    return {
        ...
        "codes": (np.asarray(ref_codes, dtype=int).tolist() if ref_codes is not None else None),
    }
...
def enroll_voice(tts, args, proto_out) -> None:
    ...
    try:
        ...
        print(
            json.dumps(
                {"ok": True, "enrolled": args.enroll_name, "voices": sorted(data["presets"])},
                ensure_ascii=False,
            ),
            file=proto_out,
            flush=True,
...
def enroll_batch(tts, args, proto_out) -> None:
    ...
    try:
        ...
        print(
            json.dumps(
                {"ok": False, "error": f"Không đọc được danh sách: {e}"}, ensure_ascii=False
            ),
            file=proto_out,
            flush=True,
        )
    ...
    for i, item in enumerate(items, 1):
        ...
        if not name or not wav:
            failed.append({"name": name or "(không tên)", "error": "thiếu tên hoặc đường dẫn"})
            continue
        if name in existing and not args.enroll_overwrite:
            skipped += 1
            continue
        # Tiến độ ra stderr — stdout chỉ dành cho dòng JSON kết quả.
        print(f"[vieneu-worker] ({i}/{total}) đang học giọng «{name}»", file=sys.stderr, flush=True)
        try:
            added[name] = _encode_one(tts, wav, args.style, args.enroll_no_denoise, item)
        except Exception as e:
    ...
    print(
        json.dumps(
            {"ok": True, "added": sorted(added), "skipped": skipped, "failed": failed},
            ensure_ascii=False,
        ),
        file=proto_out,
        flush=True,
...
def main() -> None:
    ...
    parser.add_argument(
        "--voice", default="", help="preset voice name, e.g. 'Phạm Tuyên' (required in serve mode)"
    )
    parser.add_argument(
        "--style", default="tu_nhien", choices=("tu_nhien", "tin_tuc", "doc_truyen")
    )
    parser.add_argument(
        "--model-dir", required=True, help="HF cache dir holding the downloaded model"
    )
    parser.add_argument(
        "--intra-threads",
        type=int,
    ...
    parser.add_argument(
        "--custom-voices",
        default="",
        help="JSON file with user-enrolled voices (models/vieneu/custom_voices.json)",
    )
    parser.add_argument(
        "--enroll",
        default="",
    ...
    parser.add_argument("--enroll-name", default="", help="display name of the enrolled voice")
    parser.add_argument("--enroll-gender", default="", choices=("", "male", "female"))
    parser.add_argument(
        "--enroll-region",
        default="",
    ...
    parser.add_argument(
        "--enroll-country", default="vn", help="mã quốc gia của giọng (vn, us, ...)"
    )
    ...
    parser.add_argument(
        "--enroll-batch",
        default="",
        help="học hàng loạt: tệp JSON là mảng các mục {wav, name, gender, region, country}",
    )
    parser.add_argument(
        "--enroll-overwrite", action="store_true", help="học lại cả những giọng đã có trong tệp"
    )
    ...
    if args.enroll and (not args.enroll_name or not args.custom_voices):
        print(
            json.dumps({"ok": False, "error": "--enroll cần --enroll-name và --custom-voices"}),
            file=proto_out,
            flush=True,
        )
        sys.exit(2)
    if args.enroll_batch and not args.custom_voices:
        print(
            json.dumps({"ok": False, "error": "--enroll-batch cần --custom-voices"}),
            file=proto_out,
            flush=True,
        )
        sys.exit(2)
    if not enrolling and not args.voice:
        print(json.dumps({"ok": False, "error": "--voice là bắt buộc"}), file=proto_out, flush=True)
        sys.exit(2)
    ...
    try:
        ...
        if n_custom:
            print(f"[vieneu-worker] nạp {n_custom} giọng tùy chỉnh", file=sys.stderr, flush=True)
        ...
        if args.voice not in known:
            print(
                json.dumps(
                    {
                        "ready": False,
                        "error": f"unknown voice '{args.voice}'; valid: {', '.join(known)}",
                    }
                ),
                file=proto_out,
                flush=True,
        ...
        print(
            json.dumps({"ready": False, "error": f"{type(e).__name__}: {e}"}),
            file=proto_out,
            flush=True,
        )
        return
    ...
    print(
        json.dumps({"ready": True, "backend": "onnx", "voice": args.voice}),
        file=proto_out,
        flush=True,
    )
    ...
    for line in sys.stdin:
        ...
        try:
            req = json.loads(line)
            audio = tts.infer(req["text"], voice=args.voice, style=args.style, silence_p=0.0)
            audio = trim_edges(audio)
            sf.write(req["out"], audio, tts.sample_rate)
            print(
                json.dumps({"ok": True, "duration": round(len(audio) / tts.sample_rate, 3)}),
                file=proto_out,
                flush=True,
            )
