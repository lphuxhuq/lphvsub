...
def main() -> None:
    ...
    parser.add_argument("--list", required=True, help="text file, one image path per line")
    ...
    for path in paths:
        try:
            ...
            print(f"ocr failed on {path} ({e}) — skipping frame", file=sys.stderr, flush=True)
            continue
        lines = []
        for item in result or []:
            ...
            lines.append({"text": text, "score": round(score, 3), "box": box, "top_y": top_y})
        lines.sort(key=lambda l: l["top_y"])
        n += 1
        print(
            json.dumps({"frame": path, "lines": lines}, ensure_ascii=False),
            file=proto_out,
            flush=True,
        )

    print(json.dumps({"done": True, "num_frames": n}), file=proto_out, flush=True)
