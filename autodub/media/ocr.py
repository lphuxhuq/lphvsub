...
def normalize_ocr_text(text: str) -> str:
    ...
    for ch in str(text or ""):
        ...
        elif "\u4e00" <= ch <= "\u9fff" or ch in "，。！？；：、…—“”‘’（）" or ch.isalnum():
            out.append(ch)
...
def merge_frame_texts(frame_items: list[dict], fps: float, total_frames: int) -> list[dict]:
    ...
    for item in frame_items:
        ...
        if groups:
            prev = groups[-1][-1]
            sim = difflib.SequenceMatcher(None, prev["text"], item["text"]).ratio()
        ...
        out.append(
            {
                ...
                "confidence": round(sum(g["score"] for g in group) / len(group), 3),
...
def windows_from_suspects(
    suspects: list[dict], duration_s: float, margin: float = WINDOW_MARGIN_S
...
def _extract_frames(
    video_path: str, start: float, end: float, fps: int, region_height: float, out_dir: str
    ...
    cmd = [
        ...
        video_path,
        "-vf",
        f"{_crop_filter(region_height)},fps={fps}",
        "-q:v",
        "2",
    ...
    return [(os.path.join(out_dir, p), start + i / fps) for i, p in enumerate(paths)]
...
def _run_ocr_worker(image_paths: list[str], venv_python: str) -> list[dict]:
    ...
    if not os.path.isfile(venv_python):
        raise RuntimeError(f"venv OCR chưa cài ({_OCR_SPECS_HINT}): {venv_python}")
    ...
    if proc.returncode != 0 and not frames:
        raise RuntimeError(f"OCR worker lỗi (exit {proc.returncode}): {proc.stderr[-400:]}")
    return frames
...
def _save_cache(work_dir: str, key: str, segments: list[dict]) -> None:
    ...
    save_json_atomic({"key": key, "segments": segments}, _cache_path(work_dir))


def run_selective_ocr(
    video_path: str, suspects: list[dict], settings, work_dir: str, duration_s: float | None = None
) -> list[dict]:
    ...
    if cached is not None:
        logger.info("OCR: dùng cache %d segment cho %d window(s)", len(cached), len(windows))
        return cached

    venv_python = settings.ocr_venv_python_path()
    all_segments: list[dict] = []
    for wi, (start, end) in enumerate(windows):
        out_dir = os.path.join(os.path.dirname(_cache_path(work_dir)), "ocr_frames", f"w{wi}")
        frames = _extract_frames(
            video_path, start, end, settings.ocr_fps, settings.ocr_region_height, out_dir
        )
        if not frames:
            continue
        ...
        for path, t in frames:
            ...
            frame_items.append({"t": t, "text": normalize_ocr_text(text), "score": score})
        segs = merge_frame_texts(frame_items, settings.ocr_fps, total_frames=len(frames))
        ...
        for path, _t in frames:
            try:
                ...
                logger.debug("Bỏ qua lỗi OSError trong ocr.py", exc_info=True)

    _save_cache(work_dir, key, all_segments)
    logger.info("OCR: %d window(s) → %d phụ đề", len(windows), len(all_segments))
    return all_segments


def detect_hardsub(video_path: str, settings, duration_s: float | None = None) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="hardsub_") as tmp:
        for i in range(probes + 1):
            ...
            frames = _extract_frames(video_path, t, t + 0.2, 10, settings.ocr_region_height, tmp)
            if not frames:
                continue
    ...
    is_hardsub = hits >= 3
    logger.info(
        "OCR hard-sub probe: %d/%d frame có phụ đề → %s",
        hits,
        probes + 1,
        "CÓ hard-sub" if is_hardsub else "không có",
    )
    return is_hardsub
