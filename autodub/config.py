...
class Settings:
    ...
    def load(cls, env_file: str | None = None, override: bool = False) -> Settings:
        ...
        def env_bool(key: str, default: str | bool = "false") -> bool:
            val = env(
                key, "true" if default is True else ("false" if default is False else str(default))
            )
        ...
        preset = _one_of(
            env("QUALITY_PRESET", "balanced"), ("fast", "balanced", "quality"), "balanced"
        )
        _p = _PRESETS[preset]
        ...
        return cls(
            ...
            asr_engine=_one_of(env("ASR_ENGINE", "whisper"), ("whisper", "paraformer"), "whisper"),
            ...
            asr_vad_pad_s=min(1.0, max(0.0, env_float("ASR_VAD_PAD_S", "0.3"))),
            ...
            ocr_region_height=min(0.5, max(0.05, env_float("OCR_REGION_HEIGHT", "0.18"))),
            vieneu_venv_python=env("VIENEU_VENV_PYTHON"),
            vieneu_model_dir=env("VIENEU_MODEL_DIR"),
            vieneu_voice=env("VIENEU_VOICE", "").strip(),
            vieneu_style=_one_of(
                env("VIENEU_STYLE", "tu_nhien"), ("tu_nhien", "tin_tuc", "doc_truyen"), "tu_nhien"
            ),
            # Người dùng đặt tay thì tôn trọng; chưa đặt thì tự tính theo
            # RAM trống + số nhân (xem _auto_vieneu_workers).
            vieneu_max_workers=max(
                1,
                min(
                    8,
                    env_int(
                        "VIENEU_MAX_WORKERS",
                        "3" if env("VIENEU_MAX_WORKERS") else str(_auto_vieneu_workers()),
                    ),
                ),
            ),
            parallel_workers=max(1, min(16, env_int("PARALLEL_WORKERS", auto_workers))),
            capcut_threads=max(1, min(16, env_int("CAPCUT_THREADS", "8"))),
            batch_prefetch_depth=max(1, min(5, env_int("BATCH_PREFETCH_DEPTH", "2"))),
            download_page_workers=max(1, min(4, env_int("DOWNLOAD_PAGE_WORKERS", "2"))),
            ...
            bg_duck_voice_db=min(0.0, max(-24.0, env_float("BG_DUCK_VOICE_DB", "-7.0"))),
            original_voice_duck_db=min(
                0.0, max(-30.0, env_float("ORIGINAL_VOICE_DUCK_DB", "-16.0"))
            ),
            duck_attack_ms=max(10, min(500, env_int("DUCK_ATTACK_MS", "80"))),
            duck_release_ms=max(10, min(1000, env_int("DUCK_RELEASE_MS", "140"))),
            dub_pre_roll_ms=max(0, min(80, env_int("DUB_PRE_ROLL_MS", "0"))),
            ...
            timing_max_start_drift_s=min(
                1.5, max(0.0, env_float("TIMING_MAX_START_DRIFT_S", "0.15"))
            ),
            voice_fit_min_speed=min(1.0, max(0.5, env_float("VOICE_FIT_MIN_SPEED", "0.90"))),
            voice_fit_max_speed=min(1.3, max(1.0, env_float("VOICE_FIT_MAX_SPEED", "1.15"))),
            timing_max_drift_s=min(5.0, max(0.0, env_float("TIMING_MAX_DRIFT_S", "1.5"))),
            timing_min_gap_s=min(1.0, max(0.0, env_float("TIMING_MIN_GAP_S", "0.12"))),
            timing_max_atempo=min(1.3, max(1.0, env_float("TIMING_MAX_ATEMPO", "1.1"))),
            ...
            translate_analysis=env_bool("TRANSLATE_ANALYSIS", _p["translate_analysis"]),
            translate_review=env_bool("TRANSLATE_REVIEW", _p["translate_review"]),
            ...
            auto_clean_intermediates=env_bool("AUTO_CLEAN_INTERMEDIATES", "false"),
            update_repo=env("UPDATE_REPO", "ttthanh2044/voxdub").strip(),
            support_url=env("SUPPORT_URL", "https://github.com/ttthanh2044/voxdub/issues").strip(),
            ...
            not in ("0", "false", "no"),
            translate_batch_size=max(1, min(100, env_int("TRANSLATE_BATCH_SIZE", "40"))),
            translate_direct_workers=max(0, min(8, env_int("TRANSLATE_DIRECT_WORKERS", "0"))),
            ...
            subtitle_mode=_one_of(env("SUBTITLE_MODE", "none"), ("none", "soft", "burn"), "none"),
            subtitle_preset=env("SUBTITLE_PRESET", "clean").strip() or "clean",
            subtitle_position=_one_of(
                env("SUBTITLE_POSITION", "bottom"), ("bottom", "middle", "top"), "bottom"
            ),
            ...
            subtitle_box=_one_of(env("SUBTITLE_BOX", "none"), ("none", "box"), "none"),
            subtitle_box_color=env("SUBTITLE_BOX_COLOR", "#000000"),
            subtitle_box_opacity=max(0, min(100, env_int("SUBTITLE_BOX_OPACITY", "60"))),
            subtitle_line_words=max(0, min(12, env_int("SUBTITLE_LINE_WORDS", "0"))),
            subtitle_max_lines=max(1, min(4, env_int("SUBTITLE_MAX_LINES", "2"))),
            subtitle_all_caps=env_bool("SUBTITLE_ALL_CAPS", "false"),
            subtitle_display=_one_of(
                env("SUBTITLE_DISPLAY", "sentence"), ("sentence", "karaoke"), "sentence"
            ),
            karaoke_words_per_cue=max(1, min(5, env_int("KARAOKE_WORDS_PER_CUE", "3"))),
            karaoke_effect=_one_of(
                env("KARAOKE_EFFECT", "pop"), ("pop", "fade", "karaoke", "none"), "pop"
            ),
            karaoke_highlight_color=env("KARAOKE_HIGHLIGHT_COLOR", "#FFD54A").strip() or "#FFD54A",
            karaoke_alignment=env_bool("KARAOKE_ALIGNMENT", _p["karaoke_alignment"]),
            ...
            mask_method=_one_of(env("MASK_METHOD", "blur"), ("blur", "ai_inpaint", "none"), "blur"),
            inpaint_engine=_one_of(
                env("INPAINT_ENGINE", "lama_onnx"), ("lama_onnx", "vsr_cli"), "lama_onnx"
            ),
            inpaint_device=_one_of(
                env("INPAINT_DEVICE", "auto"), ("auto", "cuda", "directml", "cpu"), "auto"
            ),
            ...
            frame_banner_height_ratio=max(
                0.08, min(0.40, env_float("FRAME_BANNER_HEIGHT_RATIO", 0.16))
            ),
        )
    ...
    def paraformer_configured(self) -> bool:
        ...
        return os.path.isfile(self.asr_venv_python_path()) and os.path.isfile(
            os.path.join(self.paraformer_model_dir_path(), "installed_ok.json")
        )
    ...
    def whisper_venv_configured(self) -> bool:
        ...
        return os.path.isfile(self.whisper_venv_python_path()) and os.path.isfile(
            os.path.join(self.whisper_model_dir_path(), "installed_ok.json")
        )
    ...
    def vieneu_configured(self) -> bool:
        ...
        return os.path.isfile(self.vieneu_venv_python_path()) and os.path.isfile(
            os.path.join(self.vieneu_model_dir_path(), "installed_ok.json")
        )
