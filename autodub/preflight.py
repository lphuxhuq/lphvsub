        ...
        try:
            ...
            results.append(
                CheckResult(
                    ...
                    advice="Bỏ qua được; nếu ứng dụng chạy sai, hãy gửi tệp log cho người hỗ trợ.",
                )
            )
    return results
...
def _check_ffmpeg(settings: Settings) -> CheckResult:
    ...
    ffmpeg_cmd = shutil.which("ffmpeg") or (local_ffmpeg if os.path.isfile(local_ffmpeg) else None)
    if not ffmpeg_cmd:
        return CheckResult(
            key="ffmpeg",
            title=title,
    ...
    if " subtitles " not in out:
        return CheckResult(
            ...
            title=title,
            level="warn",
            message="FFmpeg đang cài là bản rút gọn, thiếu bộ ghi phụ đề (libass).",
    ...
    return CheckResult(
        key="ffmpeg", title=title, level="ok", message="Bản đầy đủ, có bộ ghi phụ đề."
    )
...
def _check_ffprobe(settings: Settings) -> CheckResult:
    ...
    if shutil.which("ffprobe"):
        return CheckResult(key="ffprobe", title="FFprobe", level="ok", message="Sẵn sàng.")
    return CheckResult(
        key="ffprobe",
        title="FFprobe",
        level="fail",
        message="Máy có thể thiếu ffprobe (thường nằm cùng thư mục với ffmpeg).",
        advice="Cài lại FFmpeg bản đầy đủ — ffprobe nằm sẵn trong cùng thư mục bin.",
    )
...
def _check_disk(settings: Settings) -> CheckResult:
    ...
    return CheckResult(
        key="disk", title="Dung lượng đĩa", level="ok", message=f"Còn {free_gb:.0f} GB trống."
    )
...
def _check_ram(settings: Settings) -> CheckResult:
    ...
    if total <= 0:
        return CheckResult(
            key="ram", title="Bộ nhớ RAM", level="ok", message="Không đọc được thông số — bỏ qua."
        )
    if total < RAM_WARN_GB:
        return CheckResult(
            ...
            title="Bộ nhớ RAM",
            level="warn",
            message=f"Máy có {total:.0f} GB RAM — dưới mức khuyến nghị {RAM_WARN_GB:.0f} GB.",
            advice="Vẫn chạy được nhưng nên đóng bớt ứng dụng khác khi lồng "
            "tiếng, và trong Cài đặt giảm số luồng giọng đọc xuống 1.",
        )
    return CheckResult(key="ram", title="Bộ nhớ RAM", level="ok", message=f"{total:.0f} GB.")
...
def _check_vieneu(settings: Settings) -> CheckResult:
    ...
    if settings.vieneu_configured():
        return CheckResult(key="vieneu", title="Bộ giọng đọc VieNeu", level="ok", message="Đã cài.")
...
def _check_asr(settings: Settings) -> CheckResult:
    ...
    if settings.asr_engine == "paraformer":
        if settings.paraformer_configured():
            return CheckResult(
                key="asr", title="Bộ nghe (Paraformer)", level="ok", message="Đã cài."
            )
        return CheckResult(
            key="asr",
            title="Bộ nghe (Paraformer)",
            level="fail",
    ...
    return CheckResult(
        key="asr", title="Bộ nghe (Whisper)", level="ok", message=f"Model '{model_name}' đã có sẵn."
    )
